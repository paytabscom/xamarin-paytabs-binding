...
final class Field25519 {
    ...
    public static void reduceCoefficients(long[] output) {
        ...
        int i2 = 0;
        while (i2 < 10) {
            long j2 = output[i2] / TWO_TO_26;
            output[i2] = output[i2] - (j2 << 26);
            int i3 = i2 + 1;
            output[i3] = output[i3] + j2;
            long j3 = output[i3] / TWO_TO_25;
            output[i3] = output[i3] - (j3 << 25);
            i2 += 2;
            output[i2] = output[i2] + j3;
        }
        output[0] = output[0] + (output[10] << 4);
        output[0] = output[0] + (output[10] << 1);
        output[0] = output[0] + output[10];
        output[10] = 0;
        long j4 = output[0] / TWO_TO_26;
        output[0] = output[0] - (j4 << 26);
        output[1] = output[1] + j4;
    }
    ...
    public static long[] expand(byte[] input) {
        ...
        for (int i2 = 0; i2 < 10; i2++) {
            ...
            jArr[i2] = (((((input[iArr[i2]] & UByte.MAX_VALUE) | ((input[iArr[i2] + 1] & UByte.MAX_VALUE) << 8)) | ((input[iArr[i2] + 2] & UByte.MAX_VALUE) << 16)) | ((input[iArr[i2] + 3] & UByte.MAX_VALUE) << 24)) >> EXPAND_SHIFT[i2]) & MASK[i2 & 1];
        }
        return jArr;
    }
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public static void inverse(long[] out, long[] z2) {
        long[] jArr = new long[10];
